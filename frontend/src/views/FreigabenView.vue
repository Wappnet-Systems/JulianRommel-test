<template>
  <div class="space-y-24">
    <div id="new-documents">
      ...
      <div
        id="document-wrapper"
        class="flex max-h-64 justify-between space-x-3"
      >
        <FileCard
          v-for="file in getNewFiles"
          :key="file"
          :name="file.name"
          :uploaded="file.date"
          :type="file.type"
        ...
        <div
          v-if="getNewFiles.length == 1"
          class="p-2"
          style="flex-basis: 25%"
        ></div>
        <div
          v-if="getNewFiles.length <= 2"
          class="p-2"
          style="flex-basis: 25%"
        ></div>
        <div
          v-if="getNewFiles.length <= 3"
          class="p-2"
          style="flex-basis: 25%"
        ></div>
      </div>
    ...
    <div id="all-documents">
      <div class="flex justify-between items-baseline">
        ...
        <div class="pt-2 relative text-gray-600">
          ...
          <button
            type="submit"
            class="absolute right-0 top-0 mt-4 mr-3 text-offgray"
          >
            <span class="material-icons-outlined m-0"> search </span>
          </button>
        </div>
      </div>
  ...
  methods: {
    ...
    formatData(files) {
      for (let i = 0; i < files.length; i++) {
        let file = files[i];
        file.date =
          file.date.slice(8, 10) +
          "." +
          file.date.slice(5, 7) +
          "." +
          file.date.slice(0, 4);
        file.type = file.name
          .slice(file.name.indexOf(".") + 1, file.name.length)
          .toLowerCase();
